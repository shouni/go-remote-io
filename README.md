...
ならないこと**が、このライブラリが成立するための条件です。読む前に知っておくと高くつかないものを
挙げます（それぞれの理由は godoc にあります）。

* **何も無いプレフィックスの一覧は、空でエラーになりません。** ディレクトリの有無を確かめる用途には
  使えません。
* **区切り文字を渡さない一覧は、プレフィックス配下を再帰的に返します。** 受信量が増えるので、直下だけ
  でよければ `WithDelimiter("/")` を渡してください。
* **リモート URI は URL ではなく「スキーム + バケット + 生のキー」です。** `net/url` を通すと
  エラーにならないまま別のキーを指します。パスの操作はライブラリの `Dir` / `Join` / `BuildURI` を
  使ってください。
