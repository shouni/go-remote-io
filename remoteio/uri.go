...
// スキームの語彙はここに集約します。
//
// v1 は公開する形をプレフィックス ("gs://") 側に寄せていました。Scheme() の戻り値も
// Router の登録キーもその形だったためですが、RFC 3986 でスキームと呼ぶのは区切りを
// 含まない "gs" の方で、v1 の CLAUDE.md 自身が「公開済み API のため改名していない」と
// 断っていた箇所です。v2 は名前を正とし、区切りを足す操作はこのファイルの中だけに
// 閉じ込めます。呼び出し側が "://" を自前で連結する必要はありません。
const (
...
//
// v1 は区切り文字を指定したときだけ正規化しており、指定しなければ GCS / S3 では
// 素の前方一致（`data` が `data-archive/` にも当たる）、ローカルはディレクトリ走査、
// という食い違いが残っていました。v2 は常に正規化して、どのスキームでも同じ意味に
// します。Router が振り分け前に通しますが、ハンドラを直接呼ぶ場合のために公開しています。
...
//
// v1 の ResolvePath は常に nil のエラーを返していました。互換のためだけに残って
// いた戻り値なので落としています。
